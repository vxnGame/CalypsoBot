...
module.exports = class SetCommandPointsCommand extends Command {
	constructor(client) {
		super(client, {
			...
			description: 'Sets the amount of points earned per vxn\'s minions command used.',
	...
	run(message, args) {
		const amount = args[0];
		if (!amount || !Number.isInteger(Number(amount)) || amount < 0) {return this.sendErrorMessage(message, 0, 'Please enter a positive integer');}
		const {
