...
module.exports = class ToggleCommandCommand extends Command {
	...
	run(message, args) {
		...
		if (!command || (command && command.type == OWNER)) {return this.sendErrorMessage(message, 0, 'Please provide a valid command');}

		const { capitalize } = message.client.utils;

		if (command.type === ADMIN) {return this.sendErrorMessage(message, 0, `${capitalize(ADMIN)} commands cannot be disabled`);}
