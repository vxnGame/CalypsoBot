...
module.exports = class FeedbackCommand extends Command {
	constructor(client) {
		super(client, {
			...
			description: 'Sends a message to the vxn\'s minions Support Server\'s feedback channel.',
			type: client.types.MISC,
			examples: ['feedback We love vxn\'s minions!'],
		});
	}
	run(message, args) {
		const feedbackChannel = message.client.channels.cache.get(message.client.feedbackChannelId);
		if (!feedbackChannel) {return this.sendErrorMessage(message, 1, 'The feedbackChannelId property has not been set');}
