...
module.exports = class ClearWarnsCommand extends Command {
	...
	run(message, args) {
		...
		if (member === message.member) {return this.sendErrorMessage(message, 0, 'You cannot clear your own warns');}
		if (member.roles.highest.position >= message.member.roles.highest.position) {return this.sendErrorMessage(message, 0, 'You cannot clear the warns of someone with an equal or higher role');}
		...
		message.client.logger.info(oneLine`${message.guild.name}: ${message.author.tag} cleared ${member.user.tag}'s warns`);
