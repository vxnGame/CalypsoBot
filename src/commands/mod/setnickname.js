...
module.exports = class SetNicknameCommand extends Command {
	...
	async run(message, args) {
		...
		if (nickname.startsWith('"')) {
			...
			if (!nickname.includes('"')) {return this.sendErrorMessage(message, 0, 'Please ensure the nickname is surrounded in quotes');}
		...
		if (nickname.length > 32) {
			...
			let reason;
			if (args[1].startsWith('"')) {reason = message.content.slice(message.content.indexOf(nickname) + nickname.length + 1);}
