...
module.exports = class UnmuteCommand extends Command {
	...
	async run(message, args) {
		...
		if (member.roles.highest.position >= message.member.roles.highest.position) {return this.sendErrorMessage(message, 0, 'You cannot unmute someone with an equal or higher role');}

		let reason = args.slice(2).join(' ');
		if (!reason) reason = '`None`';
		if (reason.length > 1024) reason = reason.slice(0, 1021) + '...';

		if (!member.roles.cache.has(muteRoleId)) {return this.sendErrorMessage(message, 0, 'Provided member is not muted');}
