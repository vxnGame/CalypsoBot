...
module.exports = class ExplainPointsCommand extends Command {
	...
	run(message) {
      ...
      ' and by spending time in **voice chat**.';
		...
		if (!disabledCommands.includes('points')) {checkingPoints += `\nTo see current points, use the \`${prefix}points\` command.`;}

		if (!disabledCommands.includes('totalpoints')) {checkingPoints += ` To see overall points, use the \`${prefix}totalpoints\` command.`;}

		// The Leaderboard
		let leaderboard = '';

		if (!disabledCommands.includes('position')) {leaderboard += ` To check leaderboard standing, use the \`${prefix}position\` command.`;}

		if (!disabledCommands.includes('leaderboard')) {leaderboard += ` To see the leaderboard itself, use the \`${prefix}leaderboard\` command.`;}
