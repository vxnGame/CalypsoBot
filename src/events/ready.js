module.exports = async (client) => {
	...
	for (const guild of leftGuilds) {
		...
		client.logger.info(`vxn's minions has left ${guild.guild_name}`);
	}

	client.logger.info('vxn\'s minions is now online');
	client.logger.info(`vxn's minions is running on ${client.guilds.cache.size} server(s)`);
};
